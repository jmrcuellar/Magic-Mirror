...
Module.register("MMM-SmartWebDisplay", {
	...
	selectURL: function (direction) {
		...
		if (direction === "next") {
			...
			if (self.URLposition === self.url_list.length) {
				//quand on a atteint la fin de la liste, on repart au debut
				self.URLposition = 0;
			}
		} else if (direction === "prev") {
			self.URLposition--;

			if (self.URLposition < 0) {
				//si negatif, on revient a partir de la fin de la liste
	...
	suspend: function () {
		//function called when module is hidden
	...
	resume: function () {
		//function called when module is displayed again
	...
	notificationReceived: function (notification, payload) {
		if (notification === "USER_PRESENCE") {
			// notification sent by the module MMM-PIR-Sensor or others
	...
	GestionUpdateIntervalSWD: function () {
		if (UserPresence === true && this.ModuleiFrameHidden === false) {
			//user is present and module is displayed

			this.StartDisplay(); //on relance au dernier URL
			this.ModuleiFrameHidden = true;
		} else if (UserPresence === false && this.ModuleiFrameHidden === false && new Date().getTime() > this.startTimePir + this.shutoffDelay) {
			//user is not present AND module is displayed AND shutoffDelay is ended already : stop updating and stop video playing
